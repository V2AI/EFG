...
class CustomWDDataset(WaymoDetectionDataset):
    ...
    def __init__(self, config):
        ...
        self.num_point_features = len(config.dataset.format) \
            if self.nsweeps == 1 else len(config.dataset.format) + 1
    ...
    def transform_prebox_to_current_vel(self,pred_boxes3d,pose_pre,pose_cur,idx):
        ...
        expand_vels = np.concatenate([pred_boxes3d[:,6:8], np.zeros((pred_boxes3d.shape[0], 1))], axis=-1)
        ...
        expand_bboxes_global = np.concatenate([moved_bboxes_global[:,:3],np.ones((bboxes_global.shape[0], 1))], axis=-1)
        ...
        bboxes_pre2cur = np.concatenate([bboxes_pre2cur, pred_boxes3d[:,3:6],vels_pre2cur, pred_boxes3d[:,8:]],axis=-1)
        bboxes_pre2cur[..., 8] = bboxes_pre2cur[..., 8] + np.arctan2(pose_pre[..., 1, 0], pose_pre[..., 0,0])
        bboxes_pre2cur[..., 8] = bboxes_pre2cur[..., 8] - np.arctan2(pose_cur[..., 1, 0], pose_cur[..., 0,0])
    ...
    def __getitem__(self, idx):
        ...
        if not self.is_train:
            ...
            boxes3d_cur = np.concatenate([boxes3d[:,:6],vels_cur,boxes3d[:,-1:],
                                          scores_cur[:,None],labels_cur[:,None]],-1)
            boxes_all = []
            ...
            if 'centerpoint' in self.config.dataset.train_boxes_path:
                ...
                if self.score_thresh > 0:
                    label_mask = np.logical_and(labels>=0,(boxes_cur['pred_scores'] > self.score_thresh))
                else:
                    label_mask = labels >=0
                boxes3d = boxes_cur['pred_boxes3d'][label_mask].numpy()
                boxes3d[:, -1] = -boxes3d[:, -1] - np.pi / 2
                ...
                labels = boxes_cur['pred_labels']
                if self.score_thresh > 0:
                    label_mask = np.logical_and(labels>=1,(boxes_cur['pred_scores'] > self.score_thresh))
                else:
                    label_mask = labels>=1
                boxes3d = boxes_cur['pred_boxes3d'][label_mask]
            ...
            if boxes3d.shape[0] > self.max_roi_num:
                choice = np.random.choice(boxes3d.shape[0], self.max_roi_num, replace=False)
            ...
            if self.traj_length > 1:
                for i in range(1,self.traj_length):
                    ...
                    if 'centerpoint' in self.config.dataset.train_boxes_path:
                        ...
                        if self.score_thresh > 0:
                            label_mask = np.logical_and(labels>=0,(boxes_pre['pred_scores'].numpy() > self.score_thresh))
                        else:
                            label_mask = labels >=0
                        ...
                        if self.score_thresh > 0:
                            label_mask = np.logical_and(labels>=1,(boxes_pre['pred_scores'] > self.score_thresh))
                        else:
                            label_mask = labels >=1
                    ...
                    if i==1:
                        pred_boxes_trans = self.transform_prebox_to_current_vel(boxes3d_pre,
                                                                        ...
                                                                        pose_cur=info["veh_to_global"],idx=time_step)

                        boxes_all.append(pred_boxes_trans)
                    
                    pred_boxes = self.transform_prebox_to_current_vel(boxes3d_pre,
                                                                    pose_pre=info_pre["veh_to_global"],\
                                                                    pose_cur=info["veh_to_global"],idx=0)
                    if i==1:
