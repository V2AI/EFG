...
class TrajectoryFormer(nn.Module):
    def __init__(self, config):
        ...
        self.velboxembed = MotionEncoder(config.model.motion_input_dim,self.hidden_dim,
        ...
        encoder_layer = [TransformerEncoderLayer(self.config, d_model=self.hidden_dim, 
         nhead=self.nhead, dim_feedforward=self.dim_feedforward) for i in range(self.num_encoder_layers)]
        encoder_layer_gl = [TransformerEncoderLayerGlobalLocal(self.config, 
            d_model=self.hidden_dim, nhead=self.nhead, dim_feedforward=self.dim_feedforward) for i in range(self.num_encoder_layers)]
        encoder_norm = None
        self.encoder_fg = TransformerEncoder(encoder_layer, self.num_encoder_layers, encoder_norm,self.config)
        self.encoder_globallocal = TransformerEncoderGlobalLocal(encoder_layer_gl, 
                                            self.num_encoder_layers, encoder_norm,self.config)
    ...
    def forward_train(self, batched_inputs):
        ...
        if self.is_train:
            load_boxes3d = [torch.from_numpy(bi[1]["annotations"]['pred_boxes3d']).cuda() for bi in batched_inputs]
            load_scores = [torch.from_numpy(bi[1]["annotations"]['pred_scores']).cuda() for bi in batched_inputs]
            load_labels = [torch.from_numpy(bi[1]["annotations"]['pred_labels']).cuda() for bi in batched_inputs]
            ...
            if self.num_track >0 and det_boxes3d.shape[1] > 0:
                ...
                global_trajectory_hypothses, global_candidates = \
                self.generate_trajectory_hypothses(pred_boxes3d,det_boxes3d,traj,self.num_hypo_det,hypotheses_aug)
                ...
                loss_cls_sum, loss_reg_sum, = \
                    self.get_loss(rois,gt_boxes,point_cls,joint_cls,boxes_cls,point_reg,ious_targets,reg_targets,fg_reg_mask,fg_iou_mask)
                ...
                if gt_boxes.shape[0] > 0:
                    ...
                    loss_dict = {'loss_cls': loss_cls_sum,
                                 'loss_reg':torch.tensor([0.0]).cuda().reshape(1,-1)}


            else:
                loss_dict = {'loss_cls': torch.tensor([0.0]).cuda().reshape(1,-1),
                             'loss_reg': torch.tensor([0.0]).cuda().reshape(1,-1)}
    ...
    def forward_inference(self, batched_inputs):
        ...
        det_boxes3d = [torch.from_numpy(bi[1]["annotations"]['pred_boxes3d']).cuda() for bi in batched_inputs][0][None]
        det_scores =  [torch.from_numpy(bi[1]["annotations"]['pred_scores']).cuda() for bi in batched_inputs][0][None]
        det_labels =  [torch.from_numpy(bi[1]["annotations"]['pred_labels']).cuda() for bi in batched_inputs][0][None]
        ...
        if self.frame_id ==0:
            ...
            for i in range(instance.pred_boxes.shape[0]):
                tracks.append({
                    ...
                    'detection_name': self.WAYMO_TRACKING_NAMES[int(det_labels[0,i]-1)],
            ...
            points,trajectory_hypothese, global_candidates, joint_vels,asso_mask = \
                    self.get_point_and_trajectory(traj[:,:self.history_traj_frames-1],
                                                  traj_vels[:,:self.history_traj_frames-1],
                                                  samples,det_boxes,det_vels,det_labels,det_scores)
            
            point_feat = self.get_proposal_aware_point_feature(points.reshape(-1,points.shape[-2],points.shape[-1]),
                    trajectory_hypothese[:,0].reshape(self.batch_size,1,-1,8),self.num_track*self.num_candi)
            ...
            token = self.token.repeat(self.batch_size*(self.num_track*self.num_candi),1,1).cuda()
            ...
            output_new = {'pred_logits': det_scores, 'pred_boxes': det_boxes, 
                            "pred_labels": det_labels, "pred_vels": det_vels}
            ...
            track_new = {"matched_boxes": matched_boxes,"refined_matched_boxes":refined_matched_boxes,
                            "matched_vels":matched_vels,"matched_scores":matched_scores,
                            "matched_labels":matched_labels,"track_id": track_id}
    ...
    def hypotheses_augment(self, batch_bbox, targets):
        ...
        range_config = [[0.5, 0.1, np.pi / 12, 0.7],
        ...
        for bs_idx in range(batch_bbox.shape[0]):
            ...
            for _ in range(max_aug_iteration):
                ...
                pos_shift = torch.from_numpy(((np.random.rand(3) - 0.5) / 0.5) * range_config[idx][0]).cuda().float()
                hwl_scale = torch.from_numpy(((np.random.rand(3) - 0.5) / 0.5) * range_config[idx][1] + 1.0).cuda().float()
                angle_rot = torch.from_numpy(((np.random.rand(1) - 0.5) / 0.5) * range_config[idx][2]).cuda().float()
                aug_box3d = torch.cat(
                    [bbox[:,0:3] + pos_shift[None,:], bbox[:,3:6] * hwl_scale[None,:], bbox[:,6:7] + angle_rot[None,:]],-1).cuda()
    ...
    def get_proposal_aware_point_feature(self,src, trajectory_rois,num_rois):
        ...
        for i in range(trajectory_rois.shape[1]):
            ...
            proposal_aware_car_point = src[:,i*self.num_lidar_points:(i+1)*self.num_lidar_points,:3].repeat(1,1,9) - \
                                  corner_add_center_points.unsqueeze(1).repeat(1,self.num_lidar_points,1) 

            lwh = trajectory_rois[:,i,:,:].reshape(self.batch_size * num_rois, -1)[:,3:6].unsqueeze(1).repeat(1,
                                                                       proposal_aware_car_point.shape[1],1)
    ...
    def get_trajcetory_point_feature(self, global_trajectory_hypothses,samples):
        ...
        point_feat = self.get_proposal_aware_point_feature(point.reshape(-1,point.shape[-2],point.shape[-1]),
                global_trajectory_hypothses[:,0].reshape(self.batch_size,1,-1,8),self.num_track*candi_length)

        point_feat = point_feat.reshape(-1,self.num_lidar_points,point_feat.shape[-1])

        token = self.token.repeat(self.batch_size*self.num_track*self.num_hypo_train,1,1).cuda()
        point_feat_list = self.encoder_fg(token,point_feat)
    ...
    def get_trajectory_boxes_feature(self,traj_rois):
        ...
        len_traj, num_track, num_candi = traj_rois.shape[1], traj_rois.shape[2], traj_rois.shape[3]
    ...
    def generate_trajectory_hypothses(self,transfered_det,det_boxes3d,traj,num_hypo_det,aug_hypo=None):
        ...
        matched_id  = torch.arange(transfered_det.shape[1]).cuda().reshape(1,-1,1).repeat(batch_size,1,num_hypo_det)
    ...
    def get_cls_targets(self,pred_boxes3d,global_candidates,targets):
        ...
        for i in range(batch_size):
            ...
            if pred_boxes3d[i].shape[0] > 0 and num_gt > 0:
                ...
                rois_iou = boxes_iou3d_gpu(rois, targets[i]["gt_boxes"][:,[0,1,2,3,4,5,-1]].cuda())
                ...
                group_iou = rois_iou[torch.arange(num_track).cuda(),:,track_id].reshape(-1,self.num_hypo_train)
                ...
                ious_targets.append(torch.zeros([num_track,self.num_hypo_train]).cuda())
                fg_mask_list.append(torch.zeros([num_track]).cuda().bool())
                reg_mask_list.append(torch.zeros([num_track,self.num_hypo_train]).cuda().bool())
                gt_boxes_list.append(torch.zeros([num_track*self.num_hypo_train,7]).cuda())
    ...
    def get_loss(self,rois,gt_boxes, point_cls,joint_cls,boxes_cls,point_reg,ious_targets,reg_targets,fg_reg_mask,fg_iou_mask):
        ...
        loss_corner = get_corner_loss(point_reg.reshape(-1,7),rois.repeat(self.num_encoder_layers,1),
                                           gt_boxes.repeat(self.num_encoder_layers,1),fg_reg_mask)
    ...
    def init_trajectory(self,pose,det_boxes,det_scores,det_vels,det_labels):
        ...
        score_mask = self.class_agnostic_nms(det_boxes[0],det_scores[0].reshape(-1),
    ...
    def get_point_and_trajectory(self,traj,traj_vels,samples,det_boxes,det_vels,det_labels,det_scores):
        ...
        det_candi, det_candi_vel,asso_mask = \
        self.get_det_candi(self.pose,pred_candi,cur_vels, det_boxes,det_vels,det_labels,det_scores,self.frame_id)
        ...
        trajectory_hypothese, global_candidates, joint_vels = \
        self.genereate_trajcetory_hypotheses_inference(pred_traj,det_candi, traj,cur_vels,det_candi_vel)
    ...
    def get_pred_motion(self, traj, pred_vel=None):
        ...
        future_traj = transform_trajs_to_global_coords(future_traj_local,
    ...
    def genereate_trajcetory_hypotheses_inference(self,pred_hypo, cp_matched_boxes,traj, cur_vels, det_candi_vel):
        ...
        group_det_boxes = torch.cat([cp_matched_boxes.unsqueeze(2),torch.zeros([self.batch_size,self.num_track,1,1]).cuda()],-1)
    ...
    def update_trajectory(self,output_new,track_new):
        ...
        instance.refined_pred_boxes = torch.cat([refined_matched_boxes.cuda(),new_det_boxes.cuda()],0)
        ...
        instance.pred_classes = torch.cat([matched_labels.cuda(),new_det_labels.cuda()],0)
    ...
    def update_global_hypotheses_for_dist_asso(self,global_boxes,global_vels,instance):
        ...
        for i in range(instance.pred_boxes.shape[0]):
            tracks.append({
                ...
                'ct': global_boxes[i,:2].cpu().numpy(),
                'velocity': global_vels[i].cpu().numpy(),
                'detection_name': self.WAYMO_TRACKING_NAMES[int(instance.pred_classes[i]-1)],
                'score': instance.scores[i].cpu().numpy(),
                'box_id': i,
                'tracking_id': i,
                'label_preds': instance.pred_classes[i].cpu().numpy(),
