...
from efg.data.structures.boxes import Boxes, pairwise_iou
...
from efg.modeling.assigners.anchor_generator import ShiftGenerator
from efg.modeling.assigners.box_regression import Shift2BoxTransform
...
from efg.modeling.losses.iou2d_loss import iou_loss_v2
from efg.modeling.operators.nms import batched_nms
...
class AutoAssign(nn.Module):
    ...
    def __init__(self, config):
        ...
        pixel_mean = torch.Tensor(config.model.pixel_mean).to(self.device).view(3, 1, 1)
        pixel_std = torch.Tensor(config.model.pixel_std).to(self.device).view(3, 1, 1)
    ...
    def losses(self, shifts, gt_instances, box_cls, box_delta, box_center):
        ...
        negative_loss = negative_bag_loss(
            pred_class_probs * pred_obj_probs * (1 - pred_box_probs), self.focal_loss_gamma
        ).sum() / max(1, num_background)
    ...
    def inference(self, box_cls, box_delta, box_center, shifts, images):
        ...
        for img_idx, shifts_per_image in enumerate(shifts):
            ...
            results_per_image = self.inference_single_image(
                box_cls_per_image, box_reg_per_image, box_ctr_per_image, shifts_per_image, tuple(image_size)
            )
