...
from efg.modeling.common.weight_init import kaiming_init
...
class SepHead(nn.Module):
    def __init__(self, in_channels, heads, head_conv=64, final_kernel=1, bn=False, init_bias=-2.19, **kwargs):
...
class CenterHead(nn.Module):
    def __init__(self, cfg, logger=None, init_bias=-2.19, share_conv_channel=64, num_hm_conv=2):
        super(CenterHead, self).__init__()

        tasks = cfg.model.head.tasks
        num_classes = [len(t["class_names"]) for t in tasks]
        self.class_names = [t["class_names"] for t in tasks]
        self.code_weights = cfg.model.head.code_weights
        self.weight = cfg.model.head.loc_weight  # weight between hm loss and loc loss
        self.dataset = cfg.model.head.dataset
        self.common_heads = cfg.model.head.common_heads
        ...
        self.box_n_dim = 9 if "vel" in cfg.model.head.common_heads else 7
        ...
        for num_cls in num_classes:
            ...
            self.tasks.append(SepHead(share_conv_channel, heads, bn=True, init_bias=init_bias, final_kernel=3))
