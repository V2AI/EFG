...
from efg.data.detection_utils import check_image_size, read_image
...
from efg.data.structures.masks import BitMasks, PolygonMasks
...
@DATASETS.register()
class COCODatasetV2(BaseDataset):
    def __init__(self, config):
        super(COCODatasetV2, self).__init__(config)

        self.is_train = config.task == "train"

        self.dataset_name = config.dataset.name + ("_train" if self.is_train else "_val")
        ...
        self.with_gt = config.dataset.get("with_gt", True)
        ...
        if self.with_gt:
            if "panoptic" in self.dataset_name:
                ...
                dataset_dicts = self._load_coco_panoptic_json(
                    meta.json_file,
                    meta.image_root,
                    meta.panoptic_root,
                    meta,
            ...
            if self.is_train:
                self.dataset_dicts = self._filter_annotations(dataset_dicts)
            else:
        ...
        logger.info(f"Building data processors: {self.transforms} for Dataset {self}")
    ...
    def __getitem__(self, index):
        ...
        # data = PathManager.open(dataset_dict["file_name"], "rb").read()
        # img_array = np.frombuffer(memoryview(data), np.uint8)
        ...
        if self.with_gt:
            if "pan_seg_file_name" in dataset_dict:
                pan_seg_gt = read_image(dataset_dict["pan_seg_file_name"], "RGB")
                # apply the same transformation to panoptic segmentation
                annotations = [{"sem_seg": pan_seg_gt}]
            else:
                annotations = None
        ...
        if "pan_seg_file_name" in dataset_dict:
            ...
            pan_seg_gt = rgb2id(annotations[0]["sem_seg"])
    ...
    def _load_coco_panoptic_json(json_file, image_dir, gt_dir, meta):
        ...
        f = PathManager.open(json_file, "rb").read()
        json_info = json.loads(f)
