...
from efg._C import sort_vertices_forward
...
def get_intersection_points(polys1: torch.Tensor, polys2: torch.Tensor):
    ...
    line1 = torch.cat([polys1, polys1[..., [1, 2, 3, 0], :]], dim=2)  # n, 4, 4: Box, edge, point
...
def build_vertices(
    ...
    n = polys1.size(0)
    # (n, 4+4+16, 2)
    vertices = torch.cat([polys1, polys2, inters.view([n, -1, 2])], dim=1)
    # Bool (n, 4+4+16)
...
def oriented_box_intersection_2d(polys1: torch.Tensor, polys2: torch.Tensor):
    ...
    # build vertices
    vertices, mask = build_vertices(polys1, polys2, c12, c21, inters, mask_inter)
