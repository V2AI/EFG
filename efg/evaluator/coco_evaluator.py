...
from efg.data.datasets.coco import convert_to_coco_json
...
from efg.utils.logger import create_small_table, create_table_with_header
...
class COCOEvaluator(DatasetEvaluator):
    ...
    def __init__(self, config, output_dir, dataset=None):
        ...
        Args:
            dataset_name (str): name of the dataset to be evaluated.
                ...
                Or it must be in efg's standard dataset format
                so it can be converted to COCO format automatically.
            meta (SimpleNamespace): dataset metadata.
            config (config dict): efg Config instance.
        ...
        self._dump = True
        self.config = config
        self._tasks = self._tasks_from_config(config)
        ...
        self._kpt_oks_sigmas = config.dataset.test.keypoint_oks_sigmas
    ...
    def _tasks_from_config(self, config):
        """
        Returns:
...
def _evaluate_predictions_on_coco(coco_gt, coco_results, iou_type, kpt_oks_sigmas=None):
    ...
    if iou_type == "keypoints":
        ...
        assert len(coco_eval.params.kpt_oks_sigmas) == num_keypoints, (
            "[COCOEvaluator] The length of config.dataset.test.keypoint_oks_sigmas (default: 17) "
