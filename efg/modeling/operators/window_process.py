...
class WindowProcess(torch.autograd.Function):
    ...
    def forward(ctx, input, B, H, W, C, shift_size, window_size):
        output = _C.roll_and_window_partition_forward(input, B, H, W, C, shift_size, window_size)
    ...
    def backward(ctx, grad_in):
        ...
        grad_out = _C.roll_and_window_partition_backward(grad_in, B, H, W, C, shift_size, window_size)
...
class WindowProcessReverse(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input, B, H, W, C, shift_size, window_size):
        output = _C.window_merge_and_roll_forward(input, B, H, W, C, shift_size, window_size)
    ...
    def backward(ctx, grad_in):
        ...
        grad_out = _C.window_merge_and_roll_backward(grad_in, B, H, W, C, shift_size, window_size)
        return grad_out, None, None, None, None, None, None, None
