...
from efg.modeling.common.activation_funcs import MemoryEfficientSwish, Swish
from efg.modeling.common.batch_norm import get_norm
from efg.modeling.common.blocks import Conv2dSamePadding as Conv2d
from efg.modeling.common.blocks import MaxPool2dSamePadding as MaxPool2d
from efg.modeling.common.blocks import SeparableConvBlock
...
def build_resnet_bifpn_backbone(cfg, input_shape):
    """
    Args:
        cfg (OmegaConf): an efg config instance.
    Returns:
    ...
    norm = cfg.fpn.norm
    bn_momentum = cfg.fpn.bn_momentum
    bn_eps = cfg.fpn.bn_eps
    memory_efficient = cfg.fpn.memory_efficient_swish
    input_size = cfg.fpn.input_size
    out_channels = cfg.fpn.out_channels
    bifpn_layers = cfg.fpn.num_layers
    fuse_type = cfg.fpn.fust_type

    # when norm is "" or None, set norm = "BN"
    if not norm:
        norm = "BN"

    bottom_up = build_sparse_resnet_backbone(cfg.resnet, input_shape)
    top_block = BiFPNP6P7(
        bottom_up.output_shape()[in_features[-1]].channels,
        out_channels,
        norm,
        cfg.fpn.top_block_in_feature,
    )
    bifpn = BiFPN(
        input_size,
        bottom_up,
