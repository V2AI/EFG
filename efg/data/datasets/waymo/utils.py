...
def read_from_file(info, nsweeps=1, root_path=""):
    if "s3" not in root_path:
        ...
        info_path = root_path + "/" + info["path"]
    obj = pickle.load(PathManager.open(info_path, "rb"))
        ...
        for i in range(nsweeps - 1):
            ...
            if "s3" not in root_path:
                if not os.path.isabs(sweep["path"]):
                    sweep_path = os.path.join(root_path, sweep["path"])
            else:
                sweep_path = root_path + "/" + sweep["path"]
            sweep_obj = pickle.load(PathManager.open(sweep_path))
