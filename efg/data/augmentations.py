...
import efg
from efg.data.structures.boxes import Boxes, BoxMode, pairwise_iou
        ...
        if annotations is not None:
            for annotation in annotations:
                ...
                if "keypoints" in annotation:
                    ...
                    Args:
                        keypoints (list[float]): Nx3 float in efg Dataset format.
...
@PROCESSORS.register()
class CenterAffine(DefaultTransform):
    """
...
@PROCESSORS.register()
class RandomFlip(DefaultTransform):
    """
    ...
    def __call__(self, img, annotations=None, **kwargs):
        ...
        def process(image):
            if self.mode == "PIL":
                ...
                return Image.fromarray(image).filter(ImageFilter.GaussianBlur(radius=sigma))
            elif self.mode == "cv2":
                sigma = np.random.rand() * 1.9 + 0.1
                return cv2.GaussianBlur(np.asarray(image), (23, 23), sigma)

        if np.random.random() < self.p:
            if isinstance(img, list):
                results = [np.array(process(i)) for i in img]
            else:
                results = np.array(process(img))
...
@PROCESSORS.register()
class RandomScale(DefaultTransform):
    """
...
class IoUCropTransform(DefaultTransform):
    """
...
class CropTransform(DefaultTransform):
    """
    Perform crop operations on images.
    """
...
@PROCESSORS.register()
class RandomExtent(DefaultTransform):
    """
...
class ResizeTransform(DefaultTransform):
    """
    Resize the image to a target size.
    """
...
class TorchTransformGen(Transform):
    ...
    def __call__(self, img: np.ndarray, annotations: None, **kwargs):
        if isinstance(img, list):
            try:
                results = [np.array(self.tfm(Image.fromarray(i))) for i in img]
            except:
                results = [self.tfm(torch.tensor(i)).numpy() for i in img]
        else:
            try:
                results = np.array(self.tfm(Image.fromarray(img)))
            except:
                results = self.tfm(torch.tensor(img)).numpy()
