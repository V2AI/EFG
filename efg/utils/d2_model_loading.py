...
from .checkpoint_utils import get_missing_parameters_message, get_unexpected_parameters_message
...
def convert_c2_detectron_names(weights):
    """
    Map Caffe2 Detectron weight names to efg (the same as detectron2) names.
...
def align_and_update_state_dicts(model_state_dict, ckpt_state_dict, c2_conversion=True):
    ...
    if len(unmatched_ckpt_keys):
        logger.info(get_unexpected_parameters_message(original_keys[x] for x in unmatched_ckpt_keys))
