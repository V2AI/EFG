...
class JSONWriter(EventWriter):
    ...
    def write(self, window_size=20):
        ...
        for k, (v, iter) in storage.latest_with_smoothing_hint(window_size).items():
...
class CommonMetricPrinter(EventWriter):
    ...
    def write(self, window_size=20):
        ...
        try:
            data_time = storage.history("data_time").avg(window_size)
        except KeyError:
        ...
        losses = "  ".join(
            ["{}: {:.3f}".format(k, v.median(window_size)) for k, v in storage.histories().items() if "loss" in k]
        )
        other_metrics = "  ".join(
            [
                "{}: {:.3f}".format(k, v.median(window_size))
