...
class CenterHead(nn.Module):
    def __init__(
        ...
        self.code_weights = cfg.model.head.misc.code_weights
        self.weight = cfg.model.head.misc.weight    # weight between hm loss and loc loss
        self.dataset = cfg.model.head.misc.dataset

        self.common_heads = OmegaConf.to_container(cfg.model.head.misc.common_heads)
        ...
        self.box_n_dim = 9 if 'vel' in cfg.model.head.misc.common_heads else 7
        self.use_direction_classifier = False

        # Configurable BN
        self._norm_cfg = cfg.model.neck.norm
        ...
        self.shared_conv = nn.Sequential(
            ...
            get_norm(self._norm_cfg, share_conv_channel),
        ...
        for num_cls in num_classes:
            ...
            self.tasks.append(
                SepHead(share_conv_channel, heads, bn=self._norm_cfg, init_bias=init_bias, final_kernel=3)
            )
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            if double_flip:
                ...
                for k in preds_dict.keys():
                    ...
                    # the second one is X flip pointcloud(y=-y), the third one is Y flip pointcloud(x=-x), and the
                    # last_one is X and Y flip pointcloud(x=-x, y=-y).
                    # Also please note that pytorch's flip function is defined on higher dimensional space, so dims=[2]
                    # means that it is flipping along the axis with H length(which is normaly the Y axis), however in
                    # our traditional word, it is flipping along the X axis.
            ...
            if test_cfg.get('per_class_nms', False):
                ...
                rets.append(
                    self.post_processing(batch_box_preds, batch_hm, test_cfg, post_center_range, task_id))
